...
source ./ci/egamingcoin/matrix.sh
...
cd build-ci/egamingcoin-$BUILD_TARGET
...
if [ "$DIRECT_WINE_EXEC_TESTS" = "true" ]; then
  ...
  wine ./src/test/test_egamingcoin.exe
else
  if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
    ./src/test/test_egamingcoin --catch_system_errors=no -l test_suite
  else
