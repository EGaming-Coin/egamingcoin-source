...
const char * const BITCOIN_CONF_FILENAME = "egamingcoin.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //egamingcoin-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\EGamingCoin
    // macOS: ~/Library/Application Support/EGamingCoin
    // Unix-like: ~/.egamingcoin
#ifdef WIN32
    // Windows
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/EGamingCoin";
#else
    // Unix-like
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty egamingcoin.conf if it does not exist
