...
impl PrivateKey {
    ...
    pub fn generate_egamingcoin(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "egamingcoin_helpers")]
    pub fn generate_egamingcoin_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_egamingcoin(rng))
            .collect()
    }
