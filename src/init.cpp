 ...
 */
static const char* BITCOIN_PID_FILENAME = "egamingcoind.pid";
...
void SetupServerArgs(NodeContext& node)
{
    ...
    argsman.AddArg("-sporkaddr=<egamingcoinaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);
    ...
    argsman.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by EGamingCoin Platform, to the specified username.", ArgsManager::ALLOW_ANY, OptionsCategory::MASTERNODE);
...
/** Sanity checks
 *  Ensure that EGamingCoin Core is running in a usable environment with all
...
{
    // Make sure only a single EGamingCoin Core process is using the data directory.
...
bool AppInitMain(const CoreContext& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !fs::path(args.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if EGamingCoin Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if EGamingCoin Core is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                if (!fDisableGovernance && !args.GetBoolArg("-txindex", DEFAULT_TXINDEX) && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See /pull/1817 and /pull/1743
    ...
    // ********************************************************* Step 7d: Setup other EGamingCoin services
    ...
    // ********************************************************* Step 10a: schedule EGamingCoin-specific tasks
