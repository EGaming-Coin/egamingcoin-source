...
def egamingcoinhash(s):
    return egamingcoin_hash.getPoWHash(s)
...
# Objects that map to egamingcoind objects, which can be serialized/deserialized
...
class CBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(egamingcoinhash(r))
            self.hash = egamingcoinhash(r)[::-1].hex()
...
class CompressibleBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(egamingcoinhash(r))
            self.hash = int(egamingcoinhash(r)[::-1].hex(), 16)
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in egamingcoind indicates these should be deserialized as blocks
