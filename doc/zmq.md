...
The EGamingCoin Core daemon can be configured to act as a trusted "border
router", implementing the egamingcoin wire protocol and relay, making
...
The ZeroMQ feature in EGamingCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building egamingcoind:
...
For instance:
    ...
    $ egamingcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/egamingcoind.tx.raw \
...
These options can also be provided in egamingcoin.conf.
...
From the perspective of egamingcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into egamingcoind directly. Furthermore, no information is
...
using. EGamingCoind appends an up-counting sequence number to each
