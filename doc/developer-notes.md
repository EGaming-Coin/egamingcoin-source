...
- [Development guidelines](#development-guidelines)
    - [General EGamingCoin Core](#general-egamingcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
EGamingCoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
EGamingCoin Core is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_egamingcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_egamingcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/egamingcoind -printtoconsole
...
# A coverage report will now be accessible at `./test_egamingcoin.coverage/index.html`.
```
...
To profile a running egamingcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep egamingcoind` -- sleep 60
```
...
EGamingCoin Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`egamingcoind`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
- Net threads:
  ...
  - [ThreadSocketHandler (`d-net`)](https://doxygen.bitcoincore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 21372.
...
However, in open source software such as EGamingCoin Core, where everyone uses
...
pay attention to for reviewers of EGamingCoin Core code.

General EGamingCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: EGamingCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
be sent upstream, but bugfixes may also be prudent to PR against EGamingCoin Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `egamingcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `egamingcoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of EGamingCoin Core, the wallet was always
...
- Use *invalid* EGamingCoin addresses (e.g. in the constant array `EXAMPLE_ADDRESS`) for
