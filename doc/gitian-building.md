...
*Setup instructions for a Gitian build of EGamingCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the EGamingCoin
...
- [Building EGamingCoin Core](#building-egamingcoin-core)
...
Clone the git repositories for EGamingCoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone 
git clone https://github.com/egamingcoinpay/gitian.sigs.git
```
...
in the EGamingCoin Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building EGamingCoin Core
----------------

To build EGamingCoin Core (for Linux, macOS and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the EGamingCoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/egamingcoin/.git/
...
For example:
```bash
URL=https://github.com/crowning-/egamingcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit egamingcoin=${COMMIT} --url egamingcoin=${URL} ../egamingcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit egamingcoin=${COMMIT} --url egamingcoin=${URL} ../egamingcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit egamingcoin=${COMMIT} --url egamingcoin=${URL} ../egamingcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the egamingcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=focal on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../egamingcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/egamingcoin
SIGPATH=/some/root/path/egamingcoin-detached-sigs

./bin/gbuild --url egamingcoin=${BTCPATH},signature=${SIGPATH} ../egamingcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/egamingcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/egamingcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/egamingcoin-osx-build.assert
```
...
[egamingcoin/gitian.sigs](https://github.com/egamingcoinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
