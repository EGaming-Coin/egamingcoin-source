# Bootstrappable EGamingCoin Core Builds

This directory contains the files necessary to perform bootstrappable EGamingCoin
Core builds.
...
To build EGamingCoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      EGamingCoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/egamingcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/egamingcoinpay/guix.sigs).
...
[guix-docker]: /tree/master/contrib/guix/Dockerfile
